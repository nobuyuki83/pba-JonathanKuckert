...
public class MyHangerGD : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        for (int iter = 0; iter < 100; ++iter)
        {
            ...
            Vector3 r_pin = rot * pin;
            Vector3 r_cog = rot * cog;
            Vector3 dwdo_pin = Vector3.Cross(r_pin, penalty * (r_pin + transl - pin_goal));
            Vector3 dwdo_grav = -mass * Vector3.Cross(r_cog, gravity);
            Vector3 dwdo = dwdo_grav + dwdo_pin; // differentiation of the rotation w.r.t. rotation
            // end of edit
            // -----------------------------------
