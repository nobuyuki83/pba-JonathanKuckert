...
public class MyCloth : MonoBehaviour
{
    ...
    Vector3[] gradient_spring(Vector3[] node2xyz, float length_ini, float stiffness) {
        ...
        float C = length - length_ini; // the length differences.
        Vector3 l_vec = node2xyz[1]-node2xyz[0]; // getting vector between both points
        Vector3 gradient_0 = -stiffness * C * l_vec/length;
        // Vector3 gradient_1 = -gradient_0;
        return new Vector3[2] { gradient_0, -gradient_0 };
    }
}
