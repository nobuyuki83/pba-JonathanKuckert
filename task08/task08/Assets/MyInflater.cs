...
public class MyInflater : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        for (int i_tri = 0; i_tri < tri2vtx.Length / 3; ++i_tri)
        {
            ...
            {
                int i_vtx = node2vtx[ino];
                float3 g = -grad_volume[ino];
                // Add 3x3 constraint blocks between vertex and Lagrange multiplier (symmetric)
                float3x3 gij = OuterProduct(g, new float3(1, 0, 0));
                //float3x3 gij = OuterProduct(g, g);
                bsm.AddBlockAt(i_vtx, num_vtx, math.transpose(gij));
                bsm.AddBlockAt(num_vtx, i_vtx, gij);

                // Add contribution to RHS vector
                rhs_vector[i_vtx] -=g * lambda;
            }
...
//        Debug.Assert(false);
    ...
    static (float, float3[]) volume_tri_origin_graident(float3[] node2xyz)
    {
        ...
        gradient[0] = math.cross(node2xyz[1], node2xyz[2]);
        gradient[1] = math.cross(node2xyz[2], node2xyz[0]);
        gradient[2] = math.cross(node2xyz[0], node2xyz[1]);
        // end of editing
        // ---------------
