...
fn collide_particle_ball(
    ...
    // transfered impulse
    let relative_vel = p.velo - *ball_velo;
    let vel_normal = relative_vel.dot(&plane_norm);
    let i = (2.0 * vel_normal) / (1.0 / ball_mass + 1.0 / particle_mass);
    *ball_velo += (i / ball_mass) * plane_norm;
    p.velo -= (i / particle_mass) * plane_norm;
    // no edit from here
    ////////////////////////
}
