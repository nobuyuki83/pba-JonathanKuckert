...
//        Debug.Log($"Volume: {volume_cog.Item1}, Center of Gravity: {volume_cog.Item2}");
    ...
    void Update()
    {
        for (int i = 0; i < 30; i++)
        {
            ...
            float3 omega_dot = math.mul(math.inverse(inertia_tensor), -math.cross(omega, L));
    ...
    static float3x3 InertiaTensorForTriangleMesh(int[] tri2vtx, Vector3[] vtx2xyz) {
        ...
        for (int i_tri = 0; i_tri < tri2vtx.Length/3; i_tri++) {
            ...
            for (int j = 0; j < 4; ++j)
                for (int k = 0; k < 4; ++k)
                {
                    float coeff = volume / 20f * (1f + (j == k ? 1f : 0f));
                    res += (math.dot(array[j], array[k]) * float3x3.identity - OuterProduct(array[j], array[k])) * coeff;
                }   
            // end of edit
            // ------------------
        }
